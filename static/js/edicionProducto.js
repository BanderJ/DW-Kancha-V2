...
function validateForm(form) {
    ...
    if (!precio || precio==null) errors.push("El campo precio es obligatorio.");
...
openModalButtons.forEach(button => {
    button.addEventListener('click', () => {
        ...
        if (validateForm(form)) {
            modal.style.display = 'block'; // Muestra el modal de confirmación si el formulario es válido
        }
    });
});
...
cancelButton.addEventListener('click', () => {
    modal.style.display = 'none'; // Cierra el modal de confirmación
});

closeErrorButton.addEventListener('click', () => {
    errorModal.style.display = 'none'; // Cierra el modal de error
});
